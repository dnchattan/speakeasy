...
public abstract class ApiServer<T> : IApiServer<SocketMessage>
{
	...
	private readonly string[] SupportedContracts;
	...
	public ApiServer()
	{
		...
		List<string> supportedContracts = new();
		foreach (Type type in types)
		{
			PublicApiAttribute? attr = type.GetCustomAttribute<PublicApiAttribute>(true);
			if (attr == null)
				continue;
			supportedContracts.Add(attr.Name);
		}
		SupportedContracts = supportedContracts.ToArray();
	}

	public bool SupportsContract(string contractName)
	{
		return SupportedContracts.Contains(contractName);
	}
	...
	private void Unsubscribe(SubscriptionMessage subscriptionMessage, IApiSession<SocketMessage> session)
	{
		try
		{
			EventInfo eventInfo = Api.GetPublicApi<EventInfo>(subscriptionMessage.EventName, out string contractName);
			if (!EventHandlerDelegates.TryGetValue($"{session.Id}:{contractName}:{subscriptionMessage.EventName}", out Delegate? handler))
				return;
	...
	private async Task SendEvent(ApiMemberDefinition eventMember, EventInfo eventInfo, IApiSession<SocketMessage> session, SerializableEventArgs args)
	{
		try
		{
			...
			{
				if (EventHandlerDelegates.Remove($"{session.Id}:{eventMember.Contract}:{eventMember.PublicName}", out Delegate? handler))
				{
					eventInfo.RemoveEventHandler(this, handler);
				}
				return;
			...
			SocketMessage message = new(eventMember.Contract, "send-event", JToken.FromObject(eventMsg));
	...
	private async void CallMethod(CallMethodMessage message, IApiSession<SocketMessage> session)
	{
		...
		try
		{
			MethodInfo methodInfo = Api.GetPublicApi<MethodInfo>(message.MethodName, out contractName);
			...
			var response = new SocketMessage(contractName, "set-promise", returnValue);
			await session.SendAsync(response);
		}
		catch (Exception ex)
		{
			var response = new SocketMessage(contractName, "set-promise", message.Reject(ex));
			await session.SendAsync(response);
		}
	}
	...
	private async void GetProperty(GetPropertyMessage message, IApiSession<SocketMessage> session)
	{
		...
		try
		{
			PropertyInfo propertyInfo = Api.GetPublicApi<PropertyInfo>(message.PropertyName, out contractName);
